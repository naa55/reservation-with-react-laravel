...
export class ReservationCreate extends Component {
    ...
    fetchTables() {
        axios.get('/api/table').then((response) => this.setState({tables: response.data.data}))
    }
    ...
    handleSubmit(e) {
        ...
        axios.post('/api/reservation', {
            ...
            }).then((response) => this.props.navigate('/thankyou'))
    ...
    renderTables() {
        const tableOption = this.state.tables.map(table => <option key={table.id} name="table_id" value={table.id}>{table.name}</option> );
        return (
            <div className="m-2">
                <select name='table_id' onChange={this.handleTableChange}  className="mt-1 sm:mt-0 sm:w-1/4 rounded-md shadow-sm border-gray-300 focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50">
                        <option value="">--Select Table--</option>
                        {tableOption}
                </select>
                {this.errorMessage('table_id')}
            </div>
...
export default withNavigation(ReservationCreate)
